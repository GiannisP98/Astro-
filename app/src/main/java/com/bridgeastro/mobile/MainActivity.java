...
public class MainActivity extends Activity {
    ...
    private void ensureMsi2OnImportedEngine() {
        ...
        try {
            ...
            byte[] msiBytes = readGzipBase64Assets("msi_v2_", 4);
            ...
            // The full MSI2 engine can also be imported from the app menu.
        }
    }
    ...
    private byte[] readGzipBase64Assets(String prefix, int count) throws Exception {
        ByteArrayOutputStream joined = new ByteArrayOutputStream();
        for (int i=0;i<count;i++) {
            String name = prefix + String.format(java.util.Locale.US, "%02d.b64", i);
            try (InputStream in = getAssets().open(name)) { joined.write(readAllBytes(in)); }
        }
        byte[] gz = android.util.Base64.decode(joined.toByteArray(), android.util.Base64.DEFAULT);
